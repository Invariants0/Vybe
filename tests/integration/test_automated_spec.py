...
class TestUsers:
    ...
    def test_list_users(self, client):
        client.post(
            "/users",
            json={"username": "silvertrail15", "email": "silvertrail15@hackstack.io"},
        )
        client.post(
            "/users",
            json={"username": "urbancanyon36", "email": "urbancanyon36@opswise.net"},
        )
    ...
    def test_create_user_invalid_email(self, client):
        response = client.post(
            "/users", json={"username": "testuser", "email": "invalid-email"}
        )
    ...
    def test_update_user(self, client):
        ...
        response = client.put(
            f"/users/{user_id}", json={"username": "updated_username"}
        )
...
class TestURLs:
    ...
    def test_list_urls_filter_by_user(self, client):
        user1_response = client.post(
            "/users", json={"username": "user1", "email": "user1@example.com"}
        )
        user1_id = user1_response.get_json()["id"]

        user2_response = client.post(
            "/users", json={"username": "user2", "email": "user2@example.com"}
        )
        user2_id = user2_response.get_json()["id"]

        client.post(
            "/urls",
            json={"user_id": user1_id, "original_url": "https://example.com/user1"},
        )
        client.post(
            "/urls",
            json={"user_id": user2_id, "original_url": "https://example.com/user2"},
        )
...
class TestEdgeCases:
    ...
    def test_invalid_url_format(self, client):
        ...
        response = client.post(
            "/urls", json={"user_id": user_id, "original_url": "not-a-valid-url"}
        )
