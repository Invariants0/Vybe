...
import { Activity, AlertTriangle, Cpu, Database, ServerCrash, Terminal } from 'lucide-react';
    ...
    <main className="min-h-screen flex flex-col bg-vybe-dark text-vybe-light font-mono">
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto p-6 space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-vybe-black border-2 border-vybe-primary p-6 shadow-[8px_8px_0px_0px_#87ceeb]">
          <div>
            <h1 className="text-3xl font-heading font-extrabold text-vybe-primary mb-2">
              SRE Dashboard
            </h1>
            <div className="flex items-center gap-3">
              <Activity
                className={`w-5 h-5 ${status === 'HEALTHY' ? 'text-vybe-accent' : status === 'DEGRADED' ? 'text-vybe-primary' : 'text-[#ef4444]'}`}
              />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-vybe-black border-2 border-vybe-darkgray p-6 shadow-[8px_8px_0px_0px_#444444]">
              <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                <Activity className="w-5 h-5" /> Latency (ms)
              </h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={data}>
                    ...
                    <YAxis stroke="#e5e5e5" />
                    <Tooltip
                      contentStyle={{ backgroundColor: '#1a1d23', border: '2px solid #87ceeb' }}
                    />
                    <Line
                      type="monotone"
                      dataKey="latency"
            ...
            <div className="bg-vybe-black border-2 border-vybe-darkgray p-6 shadow-[8px_8px_0px_0px_#444444]">
              <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                <AlertTriangle className="w-5 h-5" /> Error Rate (%)
              </h3>
              <div className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={data}>
                    ...
                    <YAxis stroke="#e5e5e5" />
                    <Tooltip
                      contentStyle={{ backgroundColor: '#1a1d23', border: '2px solid #ef4444' }}
                    />
                    <Line
                      type="step"
                      dataKey="errors"
          ...
          <div className="space-y-6">
            <div className="bg-vybe-black border-2 border-[#ef4444] p-6 shadow-[8px_8px_0px_0px_#ef4444]">
              <h3 className="text-xl font-bold mb-4 text-[#ef4444] flex items-center gap-2">
                <Terminal className="w-5 h-5" /> Chaos Controls
              </h3>
              <p className="text-sm text-vybe-light/70 mb-6">
                Inject failures to test system resilience and auto-recovery.
              </p>
              ...
              <div className="space-y-4">
                <button
                  ...
                  className="w-full bg-vybe-primary/10 border-2 border-vybe-primary text-vybe-primary py-3 font-bold hover:bg-vybe-primary hover:text-vybe-black transition-colors flex items-center justify-center gap-2"
                >
                ...
                <button
                  type="button"
                  onClick={killDB}
                  className="w-full bg-[#ef4444]/10 border-2 border-[#ef4444] text-[#ef4444] py-3 font-bold hover:bg-[#ef4444] hover:text-vybe-light transition-colors flex items-center justify-center gap-2"
                >
            ...
            <div className="bg-vybe-black border-2 border-vybe-darkgray p-6 shadow-[8px_8px_0px_0px_#444444] h-[400px] flex flex-col">
              <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
                <Terminal className="w-5 h-5" /> Incident Log
              </h3>
